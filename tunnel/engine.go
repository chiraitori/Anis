...
type DomainChecker interface {
	...
	// GetRewriteIP returns a literal IPv4 address for a custom DNS rewrite,
	// or an empty string when no rewrite matches.
	GetRewriteIP(domain string) string
}
...
func (e *Engine) serveDNS(w dns.ResponseWriter, r *dns.Msg, appOverride string) {
	...
	if e.domainChecker != nil {
		if rewriteIP := net.ParseIP(e.domainChecker.GetRewriteIP(domain)); rewriteIP != nil {
			if e.standaloneRedirectToIP(w, r, rewriteIP, appName, startTime) {
				return
			}
		}
...
func (e *Engine) standaloneRedirect(w dns.ResponseWriter, r *dns.Msg, redirectDomain, appName string, startTime time.Time) bool {
	...
	return e.standaloneRedirectToIP(w, r, ip, appName, startTime)
}

func (e *Engine) standaloneRedirectToIP(w dns.ResponseWriter, r *dns.Msg, ip net.IP, appName string, startTime time.Time) bool {
	if ip.To4() == nil {
		return false
	}
...
func (e *Engine) handleDNSQuery(queryInfo *DNSQueryInfo) {
	...
	if e.domainChecker != nil {
		if rewriteIP := net.ParseIP(e.domainChecker.GetRewriteIP(domain)); rewriteIP != nil {
			if e.handleCustomRedirect(queryInfo, rewriteIP, appName, startTime) {
				return
			}
		}
...
func (e *Engine) handleCustomRedirect(queryInfo *DNSQueryInfo, ip net.IP, appName string, startTime time.Time) bool {
	if ip.To4() == nil {
		return false
	}
	response := BuildRedirectResponse(queryInfo, ip)
	...
	e.totalQueries.Add(1)
	elapsed := time.Since(startTime).Milliseconds()
	e.notifyLog(queryInfo.Domain, false, queryInfo.QueryType, elapsed, appName, ip.String(), "custom rewrite")
	return true
}
...
func logf(format string, args ...interface{}) {
	msg := fmt.Sprintf("[Anis/Go] "+format, args...)
